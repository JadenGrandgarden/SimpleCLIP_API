import asyncio
from fastapi import Request
import time
from functools import wraps

from dependency_injector.wiring import inject as di_inject
from loguru import logger

from app.services.weaviate_service import BaseService


def inject(func):
    @di_inject
    @wraps(func)
    async def async_wrapper(*args, **kwargs):
        logger.debug(f"Injecting dependencies into {func.__name__} (async)")
        result = await func(*args, **kwargs)
        injected_services = [arg for arg in kwargs.values() if isinstance(arg, BaseService)]
        if len(injected_services) > 0:
            try:
                injected_services[-1].close_scoped_session()
            except Exception as e:
                logger.error(f"Error closing session: {e}")
        return result

    @di_inject
    @wraps(func)
    def sync_wrapper(*args, **kwargs):
        logger.debug(f"Injecting dependencies into {func.__name__} (sync)")
        result = func(*args, **kwargs)
        injected_services = [arg for arg in kwargs.values() if isinstance(arg, BaseService)]
        if len(injected_services) > 0:
            try:
                injected_services[-1].close_scoped_session()
            except Exception as e:
                logger.error(f"Error closing session: {e}")
        return result

    if asyncio.iscoroutinefunction(func):
        return async_wrapper
    else:
        return sync_wrapper

async def request_debug_middleware(request: Request, call_next):
    """Middleware to log request details"""
    logger.debug(f"REQUEST: {request.method} {request.url.path}")

    # Try to log body for POST/PUT requests
    if request.method in ["POST", "PUT"]:
        try:
            # For multipart form data
            if "multipart/form-data" in request.headers.get("content-type", ""):
                form = await request.form()
                logger.debug(f"Form data keys: {list(form.keys())}")
                for key in form.keys():
                    if hasattr(form[key], "filename"):
                        logger.debug(f"File field '{key}': filename={form[key].filename}")
                    else:
                        logger.debug(f"Form field '{key}': {form[key]}")
            else:
                body = await request.json()
                logger.debug(f"Request body received")
        except Exception as e:
            logger.debug(f"Could not parse request body: {str(e)}")

    # Process the request and time it
    start_time = time.time()
    response = await call_next(request)
    process_time = time.time() - start_time

    logger.debug(f"Status: {response.status_code}, Time: {process_time:.4f}s")

    return response